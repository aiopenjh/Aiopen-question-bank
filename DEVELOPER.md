...
새 JSON 백업은 과목·단원·학습 명세·문제와 랭킹 복구 정보만 포함합니다. 풀이 기록, 학습 자료 원본, 알람 설정, API 키는 제외합니다. 복원은 기존 학습 데이터를 교체하며 구형 ZIP/JSON 전체 백업도 계속 읽습니다. 인쇄용 문제집은 현재 탭의 Blob HTML 미리보기에서 선택 과목별 A4 PDF 인쇄로 제공합니다. 새 창에 `document.write`하는 방식은 브라우저에 따라 빈 탭이 남을 수 있어 사용하지 않습니다. 워터마크 이미지 주소는 Metro 자산 모듈의 `uri`를 사용합니다.
