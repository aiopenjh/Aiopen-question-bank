...
- `src/domain/ai_client.ts`는 2026-09-23 [공식 모델 목록](https://ai.google.dev/gemini-api/docs/models)에서 확인한 정식 텍스트 모델을 3.5 Flash-Lite부터 요청한다: `gemini-3.5-flash-lite` → `gemini-3.5-flash` → `gemini-3.6-flash` → `gemini-3.7-flash` → `gemini-3.8-flash`.
- `DEFAULT_GEMINI_MODEL`은 저장값 호환성을 위해 3.5 Flash로 유지한다. 실제 첫 요청은 위 후보 목록의 3.5 Flash-Lite이며, 기존 모델 선호값도 요청 순서를 바꾸지 않는다. 검증되지 않은 모델 ID나 이미지·음성 모델을 동적으로 후보에 넣지 않는다. 새 모델 출시는 공식 지원 확인 후 목록을 갱신한다.
