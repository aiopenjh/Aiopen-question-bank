...
어떤 AI 어시스턴트든 본 저장소의 코드를 수정하거나 기능을 확장할 때 **아래 8대 불변 헌법**을 반드시 준수해야 합니다.

---

## ⚖️ 2. 8대 불변 개발 헌법 (The 8 Invariant Laws)
...
- 프롬프트 지침뿐만 아니라, 시스템 코드 레벨([question_distribution.ts](file:///c:/AI-powered%20test%20generator%20app/apps/mobile/src/domain/question_distribution.ts))에서 **수학적 무작위 알고리즘으로 1, 2, 3, 4번에 정답을 완벽히 분산**시키며, 이전 문제와 연속 동일 번호 중복을 방지합니다.
...
- 모바일 브라우저(iOS Safari / Chrome)가 입력란 포커스 시 화면을 강제 확대(Pinch-Zoom)하여 모달 창이 찌그러지거나 축소되는 버그를 방지하기 위해:
  ...
  - 뷰포트 메타태그에 `maximum-scale=1.0, user-scalable=no, shrink-to-fit=no, viewport-fit=cover`를 항상 유지하십시오.

### 8. 단일 파일 500줄 준수 및 관심사 분리 (File Length Guard)
- `App.tsx`는 화면 오케스트레이션 역할에 집중하며, 코드 길이가 500줄을 넘지 않도록 커스텀 훅과 컴포넌트로 모듈화해야 합니다.
...
| 계층 | 파일 경로 | 주요 역할 및 책임 |
| :--- | :--- | :--- |
| **진입점** | `apps/mobile/App.tsx` | 전역 화면 렌더링, 뷰포트 보호 스타일 주입, 헤더 및 탭 네비게이션 조율 (500라인 준수) |
| **출제 프롬프트** | `apps/mobile/src/domain/prompts.ts` | 공인 시험 출제위원 프롬프트, 도메인 고정(Root Domain Anchoring), 교차 분야 충돌 우선순위, 엉뚱한 입력 폴백 헌법 정의 |
...
| **커리큘럼 설계** | `apps/mobile/src/domain/curriculum_generator.ts` | 대주제+소주제 기반 국가표준 5단계 마이크로 스텝 단원 자동 생성 |
| **영구 저장소** | `apps/mobile/src/data/db.ts` | 과목(Topic), 단원(Unit), 문제(Question), 오답노트, API 키 영구 보관 (AsyncStorage) |
| **CBT 시험장** | `apps/mobile/src/features/exam/ExamScreen.tsx` | 타이머, 문항 진행 바, 4지선다 마킹, 4단계 입체 해설지(오답 분석, 개념 정리, 팁, 풀이) |
...
## 🤝 5. 인수인계 메시지 템플릿 (Handoff Prompt)

다른 AI(ChatGPT, Claude, Gemini)로 세션을 옮겨서 작업을 이어갈 때는 아래 한 줄과 함께 `AGENTS.md`를 참고하도록 지시하십시오:

> *"이 프로젝트는 Celueste CBT 플랫폼입니다. 루트의 `AGENTS.md`에 정의된 8대 불변 헌법과 파일 맵을 확인하고 동일한 원칙으로 다음 작업을 진행해 주세요."*
