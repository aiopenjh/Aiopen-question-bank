...
async function readIndexedValue(key: string): Promise<string | null> {
  return withDatabase(async (database) => {
    ...
    const value = await readRequest<unknown>(request, '학습 데이터 읽기');
    await completed;
...
async function listIndexedKeys(): Promise<string[]> {
  return withDatabase(async (database) => {
    ...
    const keys = await readRequest<IDBValidKey[]>(request, '학습 데이터 목록 읽기');
    await completed;
...
async function migrateLegacyWebStorage(): Promise<void> {
  ...
  await writeIndexedEntries([
    ...valuesToMigrate,
    [MIGRATION_MARKER_KEY, MIGRATION_MARKER_VALUE],
  ]);
...
async function getAllKeys(): Promise<string[]> {
  return (await getBackend()) === 'indexeddb'
    ? listIndexedKeys()
    : [...await NativeAsyncStorage.getAllKeys()];
}
