...
export function generateWorkbookHtml(
  ...
  const watermark = `<div class="page-watermark" aria-hidden="true">
    <div class="watermark-letters">${Array.from('Celueste').map((letter, index) =>
      `<span style="transform:translateY(${index * 19}mm)">${letter}</span>`).join('')}</div>
  </div>`;
  ...
  return `<!doctype html><html lang="ko"><head><meta charset="utf-8">
    ...
    * { box-sizing: border-box; }
    ...
    .toolbar button { border: 0; border-radius: 8px; padding: 8px 18px; background: white; color: #6e4053; font: inherit; font-weight: 700; cursor: pointer; }
    main { max-width: 210mm; margin: 20px auto; background: white; padding: 14mm 12mm; }
    .unit-page, .answer-page { position: relative; isolation: isolate; break-before: page; min-height: 220mm; display: flex; flex-direction: column; }
    main > section:first-child { break-before: auto; }
    .page-watermark { position: absolute; z-index: 0; top: 64%; left: 50%; transform: translate(-50%, -50%) translateY(33mm); width: 185mm; height: 190mm; pointer-events: none; }
    .page-watermark::before { content: ""; position: absolute; inset: 0; opacity: 0.025; background: center calc(50% + 20mm) / 180mm 180mm no-repeat url("${escapeHtml(watermarkImageUrl)}"); }
    .watermark-letters { position: absolute; left: 0; right: 0; top: 20mm; display: flex; justify-content: space-between; font-size: 110pt; font-weight: 900; color: #172550; opacity: 0.03; }
    ...
    h1 { margin: 5px 0; font-size: 18pt; color: #302832; }
    ...
    @media print { body { background: white; } .toolbar { display: none; } main { margin: 0; padding: 0; max-width: none; }
