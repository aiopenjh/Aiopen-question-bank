...
[절대적 공인 교육과정 설계 헌법 (Anti-Hallucination & Fine-Grained Progressive Curriculum)]
1. [지정 과목 100% 한정]: 반드시 지정된 [${topicName}] 과목의 공식 표준 커리큘럼에만 국한하여 설계하십시오.
2. [단원 번호 연속성 필수 준수]: 각 단원의 제목은 반드시 "${startPad}단원. [공인 단원명]"부터 시작하여 순차적으로 번호를 매겨 "${endPad}단원. [공인 단원명]"까지 총 5개 단원을 출력하십시오.
3. [촘촘한 마이크로 스텝 설계 (급격한 난이도 비약 및 건너뛰기 절대 금지)]:
   - 초보자/기초 학습자가 단원 간의 난이도 차이나 급격한 비약(건너뛰기)을 느끼지 않도록, 개념 하나하나를 아주 잘게 쪼갠 '촘촘한 마이크로 계단(Micro-Steps)' 형태로 설계하십시오.
   - [필수]: 한 단원에 너무 많은 복합 개념(예: 기본 문법과 제어문을 1단원에 몰아넣거나, 기초 자료형과 함수를 한 번에 합치는 것)을 뭉뚱그리지 마십시오.
   - [예시 (프로그래밍/파이썬)]:
     01단원. 변수 선언과 기본 입출력(print/input)
     02단원. 정수형과 실수형 숫자 연산자
     03단원. 문자열 기초와 인덱싱/슬라이싱
     04단원. 불리언(Boolean)과 비교/논리 연산자
     05단원. 리스트(List) 생성과 기초 인덱싱
     06단원. 리스트 주요 메서드와 튜플(Tuple)
     07단원. 딕셔너리(Dictionary)와 집합(Set)
     08단원. if 조건문과 pass/elif 분기 제어
     09단원. for 반복문과 range() 기초
     10단원. while 반복문과 break/continue 제어
     이처럼 학습자가 한 계단씩 안정적으로 밟아나갈 수 있도록 단원 주제를 세밀하고 촘촘하게 분할하십시오.
