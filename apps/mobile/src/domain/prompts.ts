...
9. [동일 단원 누적 출제 및 개념 범위 확장 헌법 (Comprehensive Topic Coverage & Anti-Duplicate Law)]:
   - ① [단원 내 다양한 개념 골고루 확장]: 단원(${unitTitle ? `"${unitTitle}"` : '해당 단원'})의 특정 대표 개념 하나만 계속 반복하지 마십시오. 해당 단원에 수록된 **다양하고 폭넓은 세부 개념, 원리, 다른 조문/공식/이론들을 골고루 탐색**하여 출제 범위를 체계적으로 확장하십시오.
   - ② [판박이 복사 재탕 절대 금지]: 기존에 이미 출제된 문제와 발문(지문), 묻는 내용, 정답 선지가 사실상 똑같은 무성의한 판박이 복제 문항은 100% 출제를 엄격히 금지합니다.
   - ③ [자연스러운 변형 허용]: 단원의 핵심 중요 개념을 다룰 때, 구체적 사례 제시형, 긍정/부정 비틀기 등 다른 각도로 꼬아낸 변형 문항은 자연스럽게 허용하되, 특정 개념 하나에만 편중되지 않도록 단원 전체의 다양한 지식을 안배하십시오.
