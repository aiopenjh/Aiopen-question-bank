...
8. 수식이 필요한 문제(지문/보기/해설/힌트 어디든)는 모든 수식을 반드시 $...$ 기호로 감싸고, 분수는 \\frac{분자}{분모}, 제곱근은 \\sqrt{...}, 지수는 ^{...}, 아래첨자는 _{...}, 그리스 문자·시그마·적분 등은 \\sigma, \\sum, \\int 같은 LaTeX 명령으로 씁니다. 일반 텍스트에 수식 기호를 풀어서 섞어 쓰지 마세요. (나쁜 예: "1/2와 시그마" / 좋은 예: "$\\frac{1}{2}$와 $\\sigma$")
9. deepReasoningHint(힌트)는 모든 문제에 반드시 작성해야 하는 필수 항목이며, 비워두거나 생략할 수 없습니다. 최대 1~2개의 짧은 문장(200자 이내)으로만 작성합니다. 정답 번호, 정답 문구, 최종 수치, 완성된 계산식, 정답을 도출하는 과정을 절대 포함하지 마세요. "어떤 개념을 먼저 확인해야 하는지", "어느 조건을 다시 봐야 하는지" 정도의 방향만 제시합니다. (나쁜 예: "전체 확률이 1이므로 k=1/2이고 답은 2번입니다." / 좋은 예: "확률밀도함수의 전체 구간 적분값이 무엇이어야 하는지 먼저 확인해 보세요.")
