...
- AsyncStorage의 `__celueste:native-sqlite-active`: 앱 키 접두사가 아니라서 이관·초기화 대상에서 빠진다.

1. SQLite를 연다. SQLite 완료 마커가 있으면 SQLite를 사용한다. 이때 AsyncStorage 표시가 없으면 다시 기록해 스스로 복구한다.
...
6. 일치하면 **별도 트랜잭션**으로 SQLite 완료 마커를 기록하고, 그다음 AsyncStorage 표시를 기록한다.
...
| SQLite 마커 후 ~ AsyncStorage 표시 전 | 1번에서 SQLite 사용, 표시 자동 보완 |
...
| AsyncStorage 표시는 있는데 SQLite 마커·DB가 없음 | 비정상 상태로 간주해 오류 처리 (폴백 금지) |
