...
| `clear()` | `DELETE FROM kv WHERE key NOT LIKE '__celueste:%'` + AsyncStorage에 남은 앱 키 제거 (웹 `clear`와 같은 의미). 예약 키(스키마·이관 마커)는 **남긴다** — 지우면 다음 실행에서 "표시는 있는데 마커가 없음"(6.4)으로 오판한다 |
...
완료 표시는 두 곳에 둔다.
- SQLite `kv`의 `__celueste:sqlite-migration-v1`
- AsyncStorage의 **저장소 상태 객체** `__celueste:native-storage-state`: 앱 키 접두사가 아니라서 이관·초기화 대상에서 빠진다. 키 하나에 JSON 하나라 쓰기가 원자적이다.
...
1. `activeDatabase`의 SQLite를 연다. SQLite 완료 마커가 있으면 SQLite를 사용한다. 이때 상태 객체가 없으면 `activeDatabase: "celueste.db"`로 기록해 스스로 복구한다.
2. 마커가 없으면 AsyncStorage에서 앱 키(`@cogniquest:`, `@celueste:`, 비밀 키 제외)를 `multiGet`으로 읽는다.
3. 단일 exclusive 트랜잭션에서 전체를 `kv`에 upsert하고 **커밋한다(마커는 쓰지 않는다).**
4. 커밋 후 모든 키를 다시 조회해 원본과 1:1로 비교한다.
5. 불일치하면 이번 실행은 AsyncStorage로 동작하고 다음 실행 때 재시도한다. 재시도는 원본 기준으로 다시 덮어쓰므로 멱등이다.
6. 일치하면 **별도 트랜잭션**으로 SQLite 완료 마커를 기록하고, 그다음 상태 객체를 기록한다.
...
| SQLite 마커 후 ~ 상태 객체 기록 전 | 1번에서 SQLite 사용, 상태 객체 자동 보완 |
...
- 원본은 최소 2개 릴리스 동안 보존한다. 수동 복구와 다운그레이드에 대비한 것이며, 삭제는 별도 승인을 받는다.
...
2. AsyncStorage 원본 — 최소 2개 릴리스 보존, **자동 폴백에는 사용하지 않음** (6.4). 삭제는 별도 승인
...
2. 전체 백업에 풀이·복습·도전 기록 포함 (안전 모드의 전제)
3. 임시 Expo 프로젝트로 V1~V5 실기기 확인
4. 1단계 백엔드·이관 구현
5. 장애 안전 모드 구현
6. 클로즈드 테스트
7. 2단계: 7장 기준에 해당하거나 B2B 배포 전에 진행

별도 작업: 기존 웹 이관 코드 문제(6.4 참고)는 SQLite 작업과 분리해 main 기준의 작은 수정으로 먼저 처리한다.
