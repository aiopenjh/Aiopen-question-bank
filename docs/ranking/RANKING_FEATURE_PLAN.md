...
- **초고난도 도전 왕**: 정답으로 맞춘 킬러 문항(난이도 31레벨 이상, `domain/difficulty.ts`의 "확장 학습" 구간) 중 전체 기간 최고 도달 레벨이 가장 높은 사용자
...
**메인 화면**에는 두 항목의 현재 1위만 간결하게 표시한다. 이 카드는 랭킹 창을 여는 진입점이며, 카드 자체에는 연동 버튼이나 참여 등록 입력을 두지 않는다. 랭킹 창에서 연동을 마치고 돌아오면 카드를 다시 조회해 1위를 갱신한다.
...
- 판정 기준은 기기에 저장된 정답 처리 기록 중 난이도 31레벨 이상 문제의 최고 레벨이며, 연동 시점에 기기가 계산해 전송한다.
...
1. **서버**: Cloudflare Workers + D1로 진행한다 (`RANKING_SERVER_OPTIONS.md` 권장안 승인).
