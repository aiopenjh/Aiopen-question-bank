...
- 기준 `feature/android-app@c53dc7e`, 작업 브랜치 `feature/android-apk12-fixes`. 클로드 구현을 두 차례 검토했다. 마지막 Claude 세션은 사용 한도로 시작되지 못했으므로 Codex가 **실패한 신규 테스트의 지문 한 줄만** 조사·접미가 분리되도록 정정했다. 앱 구현 코드는 클로드 수정본 그대로다.
