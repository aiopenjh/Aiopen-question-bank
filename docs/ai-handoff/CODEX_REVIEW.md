...
## Android APK 12 제보 수정 최종 검토 (2026-09-25)

- 기준 `feature/android-app@c53dc7e`, 작업 브랜치 `feature/android-apk12-fixes`, 구현·검토 커밋 `e12b2e4`. 클로드 구현을 두 차례 검토했다. 마지막 Claude 세션은 사용 한도로 시작되지 못했으므로 Codex가 **실패한 신규 테스트의 지문 한 줄만** 조사·접미가 분리되도록 정정했다. 앱 구현 코드는 클로드 수정본 그대로다.
- 최종 독립 검증: Windows `cmd.exe /c npx tsc --noEmit` 오류 0, 앱 테스트 **229개 통과·실패 0**, `git diff --check` 공백 오류 없음.
- 판정: 승인 범위 내 코드 검토 통과. `경찰서`는 계속 오답이고, `{{1}}종`에 `1종` 등 지문과 중복되는 접미만 인정한다. 현행 법령 명칭 제한은 현재 시행 규정을 묻는 경우로 한정했다. 자료 등록·사용설명서 모달은 하단 안전 영역을 반영하고 설명서 스크롤의 중첩 터치를 제거했다. 설명서의 Android/웹 구분과 AI 제공자 전송 안내를 보완했다.
- 실기기 미검증: 자료 등록 마지막 행 노출, 사용설명서 펼침 후 스크롤·닫기, 새 채점 기준이 실제 시험 화면에 반영되는지. 기존에 저장된 옛 정답지는 변경하지 않았다. 새 APK 빌드·원격 push·`main` 병합·배포는 수행하지 않았다.

---

## Android APK 12 제보 수정 1차 검토 (2026-09-25)

### 2차 확인 — 새 테스트 문장 수정 필요

- Claude 재수정 후 Windows `tsc --noEmit` 통과. 표적 테스트 13개 중 12개 통과, 1개 실패(`cloze.test.cjs:142`). 원인은 테스트 지문 `{{1}}종류와 {{2}}종류가`에서 검사기가 인접 표기를 각각 `종류와`, `종류가`로 읽는데, 테스트는 `1종류`/`2 종류`를 전체 접미 입력으로 기대한 점이다.
- 제품 요구인 `{{1}}종`에 `1종` 인정과 `{{1}}종류`에 `1종` 거부는 유지한다. **테스트 지문을 쉼표·마침표 등으로 `종류` 바로 뒤에서 끊어** 기대값이 실제 판정 범위와 일치하게 한다. 조사까지 붙은 경우의 실제 동작도 필요하면 반례로 명시하되, 이번 이슈를 해결하려고 채점 범위를 다시 넓히지 않는다.
- 프롬프트의 현행 법령 조건 한정과 설명서의 AI 제공자 전송 안내는 검토 통과. 위 테스트 픽스처만 고쳐 다시 인계한다.


- 작업 ID: `android-apk12-fixes-20260925`, 기준 `c53dc7e`, Claude 구현은 미커밋 작업 트리. 독립 실행: 앱 테스트 **228개 통과**, Windows `cmd.exe /c npx tsc --noEmit` **오류 0**, `git diff --check` 공백 오류 없음.
- 판정: **좁은 재수정 필요**. 그 외 UI 안전 영역·스크롤 구조와 새 설명서의 주요 라벨은 현재 코드와 일치한다. 실기기 동작은 미검증이다.

### 재수정 요청

1. `src/domain/grading.ts`의 `matchesWithDuplicatedSuffix`는 지문 뒤 단어 전체가 아니라 모든 앞부분(`len = 1..suffix.length`)도 정답으로 인정한다. 예를 들어 `{{1}}종류`/정답 `1`에 `1종`도 통과할 수 있어, 사용자 사례의 `{{1}}종`보다 넓다. **지문에 인접한 전체 접미 표기만 중복 입력한 경우**로 제한하고, 앞부분만 입력한 반례 테스트를 추가한다. `{{1}}종`에 `1종`·`제1종` 정답 및 Pa/Ds 오답은 유지한다. 기획상 더 넓은 접미 허용이 반드시 필요하다면 근거를 보고하고 코드 변경 없이 판단을 요청한다.
2. `src/domain/prompts.ts`의 `correctAnswers에는 현재 기준으로 맞는 표현만`은 역사·옛 법령 명칭 자체를 묻는 문제까지 금지하는 일반 규칙으로 읽힌다. **현재 시행 중인 제도·법령을 묻는 경우**로 한정한다. `current_information.ts`의 현행성 지침은 그대로 유지한다.
3. `userManualSections.tsx`의 로컬 데이터 단락에, AI 기능을 쓰면 문제·답안·선택한 교재 페이지가 연결한 AI 제공자에게 전송될 수 있다는 한 문장을 추가한다. 현재의 "외부 중앙 서버에 수집하지 않는다"와 사용자의 외부 전송 이해가 혼동되지 않게 한다. 기존 AI 데이터 전송 안내 모달과 표현을 맞추고 새로운 보장이나 저장기간은 쓰지 않는다.

- 재수정 후 Claude는 한글 커밋과 `CLAUDE_REPORT.md` 보완, `STATUS.json`을 `codex_final_review`/`codex`로 인계한다. 명령 권한이 막히면 재시도하지 말고 Codex가 커밋·검증한다.
