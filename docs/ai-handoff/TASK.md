...
## Android APK 12 실기기 제보 및 사용설명서 최신화 (2026-09-25)

- 작업 ID: `android-apk12-fixes-20260925`
- 기준: `feature/android-app@c53dc7e`, 작업 브랜치 `feature/android-apk12-fixes`. Android preview 빌드 12의 실기기 제보이며, 사용자는 아래 수정과 설명서 갱신을 승인했다. 나머지 기능은 정상이라고 확인했다.
- 역할: Claude 구현과 자체 검증, Codex 독립 검토·검증. 두 AI는 동시에 같은 코드를 수정하지 않는다.

### Claude 구현 범위

1. 빈칸형 사례 1: 사용자 답 `경찰서`는 현행 도로교통법 제80조의 면허 발급 주체인 `시·도경찰청장`과 다르므로 자동 정답으로 만들지 않는다. 기존 저장 문제·점수를 일괄 수정하지 않는다. 정답지에 섞인 옛 기관 명칭(`지방경찰청`)은 앞으로 생성하는 현행 법령 문항에서 피하도록 최소 출제 지침을 보완한다. 이미 있는 공식 근거·현행성 검증 규칙은 보존한다.
2. 빈칸형 사례 2: 지문 `{{1}}종`/`{{2}}종`, 정답 `1`·`제1`/`2`·`제2`에 사용자 입력 `1종`·`2종`을 정답으로 인정한다. 지문에 실제로 인접한 접미 글자가 중복 입력된 경우만 안전하게 허용한다. 전역 정규화나 임의 약어 추측은 금지하고 `3종`, `종` 단독, `Pa`·`Ds` 등의 기존 오답 정책을 유지한다. 맥락에 맞는 회귀 테스트를 추가한다.
3. `SourceUploadModal`의 등록 자료 마지막 줄이 Android 시스템 내비게이션 영역에 가리지 않도록 안전 영역과 스크롤 하단 여백을 반영한다. 텍스트·PDF 등록과 키보드 동작은 그대로 둔다.
4. `UserManualModal`에서 항목을 펼친 뒤 스크롤이 느리거나 반응하지 않는 문제를 고친다. 스크롤을 감싼 중첩 터치 구조와 하단 안전 영역을 점검해 최소 수정한다. 원인 판단과 미검증 사항을 보고한다.
5. `userManualSections.tsx`를 현재 Android 코드와 화면 라벨에 대조해 낡은 설명을 갱신한다. 특히 빈칸·주관식/객관식 유형 선택, 채점 미완료·부분점수, PDF 저장·재연결, 백업·복원, Android 설치 앱과 웹 전용 안내를 확인한다. 실제 코드로 검증할 수 없는 기능·보장 표현은 새로 쓰지 않는다. 기존 문서 구조·톤은 가능한 범위에서 유지한다.

### 완료 조건과 경계

- 승인 범위의 관련 코드와 사용설명서만 변경한다. 다른 정상 기능, 저장 스키마, 과거 문제/풀이 데이터, 랭킹 점수, 새 의존성은 건드리지 않는다.
- 가능한 최소 회귀 테스트와 Windows 타입 검사를 실행해 결과를 보고한다. 실기기 재확인이 필요한 내용은 통과라고 쓰지 않는다.
- 한글 커밋 메시지를 사용한다. `CLAUDE_REPORT.md`에 변경 파일·판단 근거·검증 결과·미검증 사항을 적고 `STATUS.json`을 `codex_review`, `activeAgent: codex`로 넘긴다.
- 원격 push, `main` 병합, EAS 빌드, 웹·스토어 배포는 하지 않는다. Codex 검토 뒤 별도 단계다.

---

## 이전 완료 작업: Android 최신 테스트 코드에 학습 데이터 보존 수정 통합 (2026-09-25)
