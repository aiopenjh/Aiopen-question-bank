...
- 사용자 실기기 재시험에서 빈칸형 두 번째 답안 칸과 설정의 API 키 입력칸이 여전히 키보드에 가렸다. 백업·복구 등 나머지 기능은 정상이라고 확인했다.
- Codex가 이전의 자동 스크롤 방식 대신 입력칸의 실제 화면 위치와 키보드 윗선을 비교해 겹친 거리만큼 스크롤하고, 키보드 높이만큼 ScrollView 하단 여유 공간을 만든다. 시험·설정 화면에만 적용하며 저장 데이터·채점 기준·백업 경로는 변경하지 않는다.
- 기준: `origin/feature/android-app@3c35571`; 구현 커밋 `33096e8`. Windows 타입 검사와 앱 테스트 213개 통과. EAS preview APK 실기기 재검증 필요.
- 이번에는 Claude 세션 한도가 있어 Codex가 범위 안의 UI 수정과 검증을 직접 수행했다. 운영 `main` 병합·Google Play 정식 배포는 하지 않는다.
